...
new const AUTHOR[]  = "hlstriker"; //Nombre Original del Author ¬¬
...
public CmdShoopSlay(iClient, iLevel, iCid)
{
    ...
    log_amx("Cmd: ^"%s<%d><%s><>^" fea ^"%s<%d><%s><>^"", szName, get_user_userid(iClient), szAuthID, szName2, get_user_userid(iVictim), szAuthID2);
...
ShoopSlay(iVictim)
{
    ...
    else
    {
        ...
        ExecuteHam(Ham_TakeDamage, iVictim, 0, 0, 9999.0, 0);
    }
}
...
public fwd_Think(iEnt)
{
    ...
    if(pev(iEnt, pev_iuser4) == SHOOPS_ID)
    {
        ...
        if(iThinkNum == NUM_EXPLOSIONS)
        {
            ...
            set_pdata_float(iVictim, POWERUP_GOD_OFFSET, 0.0, POWERUP_GOD_LINUX_DIFF); // Remove god in TFC when set with a power up
            set_pev(iVictim, pev_takedamage, DAMAGE_AIM);
            ExecuteHam(Ham_TakeDamage, iVictim, iEnt, iEnt, 9999.0, 0);
...
SpawnShoop(iVictim, const Float:flShoopOrigin[3])
{
    ...
    set_pev(iShoop, pev_classname, "Headshot FEA");
