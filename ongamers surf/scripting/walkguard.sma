...
public ZoneTouch(player, zone) {
	...
	if ( (ZONEMODE:zm == ZM_CAMPING) || ((ZONEMODE:zm == ZM_CAMPING_T1) && (userteam == 1)) || ((ZONEMODE:zm == ZM_CAMPING_T2) && (userteam == 2)) ) {
		if (!camping[player]) {
			set_hudmessage(random_num(25, 225), random_num(25, 225), random_num(25, 225), -1.0, 0.5, 0, 0.0, 1.1, 0.0, 0.0, 4)
			show_hudmessage(player, "%L", player, "WALKGUARD_CAMPING_INIT")
...
public ZoneModeKill(player) {
	...
	for(new i = 0; i < 5; i++) {
		set_hudmessage(random_num(25, 225), random_num(25, 225), random_num(25, 225), -1.0, 0.5, 0, 0.0, 1.1, 0.0, 0.0, 4)
		show_hudmessage(player, "%L", player, "WALKGUARD_KILL_MESSAGE")
	}
...
public ZoneModeCamper(player) {
	...
	{
		set_hudmessage(random_num(25, 225), random_num(25, 225), random_num(25, 225), -1.0, 0.5, 0, 0.0, 1.1, 0.0, 0.0, 4)
		show_hudmessage(player, "%L", player, "WALKGUARD_CAMPING_DAMG")
	...
	{
		set_hudmessage(random_num(25, 225), random_num(25, 225), random_num(25, 225), -1.0, 0.5, 0, 0.0, 1.1, 0.0, 0.0, 4)
		show_hudmessage(player, "%L", player, "WALKGUARD_CAMPING_TIME", left)
	}
}
